...
$id_tes_kepribadian_kode = 'kepribadian';
...
// Ambil id_tes dari master_tes berdasarkan kode_tes (bukan hardcode ID)
$q_tes = mysqli_query($koneksi, "SELECT id_tes FROM master_tes WHERE kode_tes = '$id_tes_kepribadian_kode' LIMIT 1");
$row_tes = $q_tes ? mysqli_fetch_assoc($q_tes) : null;
if (!$row_tes) {
    die("Konfigurasi Tes Kepribadian belum terdaftar di tabel master_tes. Jalankan migrasi_tes_kepribadian.sql terlebih dahulu.");
}
$id_tes_kepribadian = (int) $row_tes['id_tes'];
...
if (isset($_POST['submit'])) {
    ...
    $q_sesi = "INSERT INTO sesi_tes (id_tes, id_siswa, status, started_at)
                 VALUES ('$id_tes_kepribadian', '$id_siswa', 'finished', '$tanggal_tes')";
    ...
    foreach ($_POST as $key => $jawaban_kode) {
        if (strpos($key, 'soal_') === 0) {
            ...
            $jawaban_kode = strtoupper(mysqli_real_escape_string($koneksi, $jawaban_kode));

            $q_jawaban = "INSERT INTO jawaban_kepribadian (id_sesi, id_soal, jawaban)
            ...
            if (isset($bobot_skor[$jawaban_kode])) {
                $skor_total += $bobot_skor[$jawaban_kode];
            }
            $detail_jawaban[$id_soal] = $jawaban_kode;
        }
    }

    if ($success) {
        // Tentukan tipe kepribadian berdasarkan rentang skor_min-skor_max
        // di tabel keterangan_kepribadian (bukan hardcode di PHP, supaya
        // guru bisa ubah rentang/label lewat database kalau perlu).
        $q_tipe = mysqli_query($koneksi, "
            SELECT kode_tipe FROM keterangan_kepribadian
            WHERE $skor_total BETWEEN skor_min AND skor_max
            LIMIT 1
        ");
        $row_tipe = $q_tipe ? mysqli_fetch_assoc($q_tipe) : null;
        $kode_tipe = $row_tipe ? $row_tipe['kode_tipe'] : null;

        $json_detail_esc = mysqli_real_escape_string($koneksi, json_encode($detail_jawaban, JSON_UNESCAPED_UNICODE));
        $kode_tipe_sql = $kode_tipe ? "'" . mysqli_real_escape_string($koneksi, $kode_tipe) . "'" : "NULL";

        $q_hasil = "INSERT INTO hasil_kepribadian (id_sesi, id_siswa, tanggal_tes, skor_total, tipe, json_detail)
                    VALUES ($id_sesi, $id_siswa, '$tanggal_tes', $skor_total, $kode_tipe_sql, '$json_detail_esc')";
    ...
    if ($success) {
        ...
        header("Location: hasil_kepribadian.php?id_hasil=$id_hasil&cleanup=true");
        exit;
    } else {
        mysqli_rollback($koneksi);
...
$query_soal = mysqli_query($koneksi, "
    SELECT id_soal, nomor, pernyataan, opsi_a, opsi_b, opsi_c, opsi_d
    FROM soal_kepribadian
    WHERE aktif = 1
    ORDER BY nomor ASC
");
$all_soal = mysqli_fetch_all($query_soal, MYSQLI_ASSOC);
...
<head>
    ...
    <title>Tes Kepribadian | BK SMKN 2 Banjarmasin</title>
    ...
    <style>
        ...
        .answer-label input[type="radio"] { position: absolute; opacity: 0; width: 0; height: 0; }
...
<body class="min-h-screen p-3 sm:p-5 lg:p-8 flex flex-col justify-center lg:block">
    ...
    <div class="max-w-6xl mx-auto w-full bg-white p-4 sm:p-6 lg:p-8 rounded-xl shadow-2xl">
        ...
        <div class="mb-5 sm:mb-6 border-b pb-4 sm:pb-5">
            <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-800">Tes Kepribadian Siswa SMK</h1>
            <p class="text-sm text-gray-600 font-medium mt-1">Pilih jawaban yang paling sesuai dengan diri Anda untuk setiap pernyataan.</p>
                ...
                <div class="bg-white p-4 rounded-xl shadow-md lg:shadow-lg sticky lg:top-4 border border-gray-100">
                    ...
                    <div class="lg:hidden scrollable-nav-mobile-wrapper">
                        <div id="navContainerMobile" class="scrollable-nav-mobile flex gap-2">
                            <?php for ($i = 1; $i <= $total_soal; $i++) { ?>
                                <button type="button" data-question-id="<?= $all_soal[$i - 1]['id_soal']; ?>" data-q-index="<?= $i - 1; ?>" id="nav-m-<?= $i; ?>"
                                    class="nav-button text-center bg-gray-100 text-gray-600 border border-gray-300"
                                    onclick="showQuestion(<?= $i - 1; ?>)"><?= $i; ?></button>
                            <?php } ?>
                            <button type="button" data-question-id="final" data-q-index="<?= $total_soal; ?>" id="nav-m-<?= $total_cards; ?>"
                                class="nav-button text-center bg-indigo-600 text-white border border-indigo-600 px-3 w-auto"
                                onclick="showQuestion(<?= $total_soal; ?>)" style="width: 4.5rem; height: 2.25rem;">Kirim</button>
                        </div>
                    ...
                    <div id="navContainerDesktopWrapper" class="hidden lg:block scrollable-nav-desktop-wrapper">
                        <div id="navContainerDesktop" class="grid grid-cols-5 gap-2 scrollable-nav-desktop">
                            <?php for ($i = 1; $i <= $total_soal; $i++) { ?>
                                <button type="button" data-question-id="<?= $all_soal[$i - 1]['id_soal']; ?>" data-q-index="<?= $i - 1; ?>" id="nav-d-<?= $i; ?>"
                                    class="nav-button w-full bg-gray-100 text-gray-600 border border-gray-300"
                                    onclick="showQuestion(<?= $i - 1; ?>)"><?= $i; ?></button>
                            <?php } ?>
                            <button type="button" data-question-id="final" data-q-index="<?= $total_soal; ?>" id="nav-d-<?= $total_cards; ?>"
                                class="nav-button w-full bg-indigo-600 text-white border border-indigo-600 col-span-5 hover:bg-indigo-700"
                                onclick="showQuestion(<?= $total_soal; ?>)" style="height: 2.5rem;">Kirim Jawaban</button>
                        </div>
            ...
            <div class="lg:w-3/4 order-2 lg:order-2">
                <div id="questionWrapper">
                    ...
                    foreach ($all_soal as $soal_item) {
                        $opsi = ['A' => $soal_item['opsi_a'], 'B' => $soal_item['opsi_b'], 'C' => $soal_item['opsi_c'], 'D' => $soal_item['opsi_d']];
                        ?>
                        <div id="q-<?= $question_number - 1; ?>" class="question-card bg-white p-5 sm:p-6 rounded-xl shadow-lg border border-gray-100 <?= $question_number == 1 ? 'active' : ''; ?>">
                            ...
                            <div class="space-y-3 mt-6">
                                <?php foreach ($opsi as $label => $teks): ?>
                                    <label class="answer-label block cursor-pointer">
                                        <input type="radio" name="soal_<?= $soal_item['id_soal']; ?>" value="<?= $label; ?>" onchange="handleRadioChange(this);" required>
                                        <span class="option-code"><?= $label; ?></span>
                                        <span class="font-medium text-gray-700 radio-text text-wrapper"><?= htmlspecialchars($teks); ?></span>
                                    </label>
                                <?php endforeach; ?>
                            </div>

                            <div class="flex justify-between mt-8">
                                <button type="button" id="prev-btn-<?= $question_number - 1; ?>"
                                ...
                                <button type="button" id="next-btn-<?= $question_number - 1; ?>"
                                ...
                                <button type="submit" name="submit" id="submit-soal-<?= $question_number - 1; ?>"
                                    ...
                                    style="display:none;">Kirim Jawaban</button>
                            </div>
                        </div>
                    <?php $question_number++; } ?>
                    ...
                    <div id="q-<?= $total_soal; ?>" class="question-card bg-white p-5 sm:p-6 rounded-xl shadow-lg border border-gray-100">
                        ...
                        <div class="flex justify-between mt-8">
                            <button type="button" id="prev-btn-<?= $total_soal; ?>"
...
<script>
    ...
    function confirmExit() {
        return confirm("Apakah Anda yakin ingin mengakhiri tes? Progres Anda akan disimpan sementara. Tekan OK untuk menyimpan dan keluar, atau Batalkan untuk melanjutkan.");
    }
